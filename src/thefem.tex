...
The starting point is the balance of momentum, stress symmetry and traction definition on the boundary are given by
\begin{equation} \label{eq:pde}
\begin{split}
&\sigma_{ij,j} + {f_i} = \rho\ddot{u}_i \quad \forall\ \vb{x} \in V,\, t \in \mathbb{R}^{+}\\
 &\sigma_{ij}=\sigma _{ji}\\
& t_i^{\hat n} = \sigma_{ij} \hat{n}_{j} \quad \forall\ \vb{x} \in S \enspace .
\end{split} 
\end{equation}

The simplest constitutive\footnote{A constitutive relation is a relation between two physical quantities (especially between kinetic and kinematic quantities) that is specific to a material or substance, and approximates the response of that material to external stimuli, usually as applied fields or forces.\cite{wiki:constitutive_relation}.} equation in solid mechanics is to consider a linear dependence between stress and strains, and it is called Hooke's law\footnote{Despite the name of \emph{law} used, this relation is not always valid, but is a good approximation for small strains.}
...
\begin{align*}
\sigma_{ij} &= \mu ( u_{i,j} + u_{j,i}) + \lambda u_{k,k}\delta _{ij}\\
\sigma_{ij,j} &= \mu u_{i,jj} + \mu u_{j,ij} + \lambda u_{k,kj}\delta_{ij}\\
\sigma_{ij,j} &= \mu u_{i,jj} + (\lambda  + \mu) u_{j,ij}
\end{align*}
from which
...
If we consider that $u_i$ is not a function of time, we obtain the static version of the equation, i.e,
\begin{equation}
\begin{split}
...
is a Neumann boundary condition on $u_i$.
