...
\[u = \bar u \quad for \quad {\vec x} \quad in \quad S_u\] (Escential boundary conditions)
...
On the other hand, in order to validate (or test) the correctness of the approximated or propossed trial functions $u$ it is also necessary to introduce test functions $w$ which are arbitrary except that they satisfy the following conditions:
...
In \cref{eq:solution} the condition $\tau_{xy}=0$ is due to the symmetrie in the problem.
...
\subsubsection*{Problem 3.15 (In Bathe's book)}\todo{add the figure corresponding to this problem}
